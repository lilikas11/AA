...
def create_plots(data):
    ...
    for filename, measurements in data.items():
        ...
        if size not in graph_data:
            graph_data[size] = {'vertices': [], 'configs': {k: [] for k in k_values}}
        
        graph_data[size]['vertices'].append(vertices)
        for measurement in measurements:
            graph_data[size]['configs'][measurement['k']].append(measurement['configs_tested'])
    ...
    for k in k_values:
        ...
        for size in colors.keys():
            if size in graph_data:
                ...
                configs = [y for _, y in sorted(zip(graph_data[size]['vertices'], 
                                                graph_data[size]['configs'][k]))]
                ax.plot(vertices, configs, 'o-', 
label=f'Density {size}',                        color=colors[size])
        ...
        ax.grid(True)
        ax.legend()
        ax.set_ylim(0, 5000)  # Limitar o eixo y a 100
